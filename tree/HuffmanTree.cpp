...
// 选择两个权值最小的 结点 下标 s1 s2 为这两个结点的下标 parent 为当前结点的下标
void selectMinWeight(HTNode* ht, int& s1, int& s2, int parent) {
    // 找到两个权值最小的结点下标
    s1 = s2 = 0;
    for (int i = 1; i < parent; i++) { // 从 1 开始使用 到 parent - 1
